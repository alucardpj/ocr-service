...
- 🎯 Entry points to execute your program using `python -m <ocr_service>` or `$ ocr_service` with basic CLI argument parsing.
...
[![codecov](https://codecov.io/gh/alucardpj/ocr-service/branch/main/graph/badge.svg?token=ocr-service_token_here)](https://codecov.io/gh/alucardpj/ocr-service)
[![CI](https://github.com/alucardpj/ocr-service/actions/workflows/main.yml/badge.svg)](https://github.com/alucardpj/ocr-service/actions/workflows/main.yml)

Awesome ocr_service created by alucardpj

## Install

from source
```bash
git clone https://github.com/alucardpj/ocr-service ocr_service
cd ocr_service
make install
```
...
```bash
pip install ocr_service
```

## Executing

```bash
$ ocr_service run --port 8080
```

or

```bash
python -m ocr_service run --port 8080
```

or

```bash
$ uvicorn ocr_service:app
```
...
```bash
❯ ocr_service --help
Usage: ocr_service [OPTIONS] COMMAND [ARGS]...
...
```bash
❯ ocr_service create-user --help
Usage: ocr_service create-user [OPTIONS] USERNAME PASSWORD
...
```bash
ocr_service create-user admin admin --superuser
```
...
Run with `ocr_service run` and access http://127.0.0.1:8000/docs

![](https://raw.githubusercontent.com/rochacbruno/fastapi-project-template/master/docs/api.png)


**For some api calls you must authenticate** using the user created with `ocr_service create-user`.
...
-----------------------------------------------------
ocr_service/__init__.py              4      0   100%
ocr_service/app.py                  16      1    94%
ocr_service/cli.py                  21      0   100%
ocr_service/config.py                5      0   100%
ocr_service/db.py                   10      0   100%
ocr_service/models/__init__.py       0      0   100%
ocr_service/models/content.py       47      1    98%
ocr_service/routes/__init__.py      11      0   100%
ocr_service/routes/content.py       52     25    52%
ocr_service/routes/security.py      15      1    93%
ocr_service/routes/user.py          52     26    50%
ocr_service/security.py            103     12    88%
-----------------------------------------------------
TOTAL                               336     66    80%
...
> `dynaconf_merge` is a boolean that tells if the settings should be merged with the default settings defined in ocr_service/default.toml.

### As environment variables
```bash
export ocr_service_KEY=value
export ocr_service_KEY="@int 42"
export ocr_service_KEY="@jinja {{ this.db.uri }}"
export ocr_service_DB__uri="@jinja {{ this.db.uri | replace('db', 'data') }}"
```
...
```bash
ocr_service_ENV=production ocr_service run
```
